...
void Huffman_::analyse(std::vector<char>& data)
{
...
void Huffman_::generateCode(HuffNode_t* node)
{
  if(node->right_ != nullptr)
  {
...
void Huffman_::getDataCode(std::vector<char>& data, std::vector<char>& out)
{
...
void Huffman_::getFile(std::vector<char>& data, std::string& out)
{
  out = "";
  BitFileGetter bit(8);
  bit.set(data);
